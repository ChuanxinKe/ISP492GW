...
# MAIN : --------------------
mission0=td.Mission('开始数据获取')

mission1=td.Mission('从某神秘网址获取,不知道哪年的财富500强CSV数据')
...
mission2=td.Mission('就排名尾部[tail()]的5个财富500强企业,逐个去Yahoo.com抓取历史收盘数据')
counter1=td.counter()
...
mission3=td.Mission('输出数据示例')
td.printfive (results,'瞅瞅前五个收盘数据长啥样')
mission3.end()

mission4=td.Mission('简单格式化并导出CSV')
out=pd.DataFrame(data=results)
out.to_csv('D:/out.csv',encoding='gbk')
mission4.end()
