...
mission1=td.Mission('Filter words list from csv data')
key_words = pd.read_csv(kwpath.path)
filter_list = ""
print(key_words)
for index,row in key_words.iterrows():
    filter_list = filter_list+row['keywords'] +'|'
filter_input = filter_list[:-1] #delete the last '|'
df_fl=df.loc[df['tweets'].str.contains(filter_input,flags=re.IGNORECASE, regex=True)]
